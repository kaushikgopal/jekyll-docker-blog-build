...
ENV NODE_HOME /usr/local/node_modules
RUN mkdir -p $NODE_HOME && chmod 777 $NODE_HOME
...
RUN addgroup -g 1000 node \
    ...
    && ARCH= && alpineArch="$(apk --print-arch)" \
      && case "${alpineArch##*-}" in \
        x86_64) \
          ARCH='x64' \
          CHECKSUM="9b652a509823aad1deabb4def231ffff6220d111bf75ea91bf49352792c97122" \
          ;; \
        *) ;; \
      esac \
  && if [ -n "${CHECKSUM}" ]; then \
...
RUN npm install -g --prefix $NODE_HOME/.. firebase-tools@8.4.1 --unsafe-perm --verbose

CMD bundle exec jekyll --version && /usr/local/bin firebase --version
